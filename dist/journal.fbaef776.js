...
})({"cLhP7":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "06aa9becfbaef776";
"use strict";
...
},{}],"kTqHM":[function(require,module,exports,__globalThis) {
...
async function fetchPokemon(id) {
    ...
    try {
        ...
        const pokemon1 = await response.json(); // Umwandlung der Antwort in ein JSON-Objekt
...
function getPokemonByName(pokemonName) {
    ...
    const pokemon1 = favorites.find((fav)=>fav.name === pokemonName);
    return pokemon1 || null;
}
function addNoteToFavorite(selectedPokemon1) {
    const favorites = JSON.parse(localStorage.getItem('favorites')) || {};
    const updatedFavorites = favorites.map((fav)=>fav.name === selectedPokemon1.name ? selectedPokemon1 : fav);
    localStorage.setItem('favorites', JSON.stringify(updatedFavorites));
}
function pokemonCardCreator(pokemon1) {
    ...
    pokemonName.textContent = pokemon1.name.toUpperCase();
    pokemonName.classList.add('text-xl', 'font-bold');
    const pokemonHP = document.createElement('span');
    const hpStat = pokemon1.stats.find((stat)=>stat.stat.name === 'hp').base_stat;
    ...
    const types = pokemon1.types.map((type)=>type.type.name).join(', ').toUpperCase();
    ...
    favoriteButton.addEventListener('click', ()=>{
        if (favoriteButton.style.color === 'gold') {
            ...
            removeItemFromFavorites(pokemon1.name);
        } else {
            favoriteButton.style.color = 'gold';
            // call function to add it into the favorites list
            addToFavorites(pokemon1.name);
        }
    });
    ...
    pokemonImage.src = pokemon1.sprites.other.showdown.front_shiny;
    pokemonImage.alt = pokemon1.name;
    pokemonImage.style.width = '150px';
    imageDiv.appendChild(pokemonImage);
    // Attack and Defense
    const statsDiv = document.createElement('div');
    statsDiv.classList.add('text-sm', 'mb-4');
    const attackStat = pokemon1.stats.find((stat)=>stat.stat.name === 'attack').base_stat;
    const defenseStat = pokemon1.stats.find((stat)=>stat.stat.name === 'defense').base_stat;
    ...
    abilitiesDiv.innerHTML = `
        ...
        <ul>
            ${pokemon1.abilities.map((ability)=>`<li>- ${ability.ability.name}</li>`).join('')}
        </ul>
    `;
    // height and weight
    const dimensionsDiv = document.createElement('div');
    dimensionsDiv.classList.add('text-sm', 'mb-4');
    dimensionsDiv.textContent = `Height: ${(pokemon1.height / 10).toFixed(1)} m | Weight: ${(pokemon1.weight / 10).toFixed(1)} kg`;
    // ----- Personal note section
    const noteDiv = document.createElement('div');
    noteDiv.classList.add('text-sm', 'mb-4', 'text-left');
    // Load favorites from localStorage
    const favoritesData1 = JSON.parse(localStorage.getItem('favorites')) || {};
    selectedPokemon = getPokemonByName(pokemon1.name);
    ...
    if (!selectedPokemon.noteFlag) {
        ...
        saveNoteButton.addEventListener('click', ()=>{
            ...
            } else alert('Please enter a valid note.');
        });
        noteDiv.appendChild(noteLabel);
        noteDiv.appendChild(noteInput);
        noteDiv.appendChild(saveNoteButton);
    } else // Show saved note if it exists
...
async function displayPokemons() {
    const pokemons = favoritePokemons.map((pokemon1)=>fetchPokemon(pokemon1.name));
    // Schleife, um die favoritePokemons auszulesen
    pokemons.forEach((pokemon1)=>{
        pokemon1.then((pokemon1)=>{
            // Nur fortfahren, wenn die Daten erfolgreich abgerufen wurden
            if (pokemon1) pokemonContainer.appendChild(pokemonCardCreator(pokemon1));
        });
    });
}
displayPokemons();

},{}]},["cLhP7","kTqHM"], "kTqHM", "parcelRequire94c2")

//# sourceMappingURL=journal.fbaef776.js.map
