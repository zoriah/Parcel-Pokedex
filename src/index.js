...
pokemonSearch.addEventListener('input', function () {
  if (pokemonSearch.value.toLowerCase().trim() === '') {
    // searchFlag = false;
    displayPokemons();
  }
});
...
function pokemonCardCreator(pokemon) {
  ...
  pokeTypeUndfavorite.classList.add(
    'w-full',
    'h-10',
    'flex',
    'items-center',
  ...
  dimensionsDiv.textContent = `Height: ${(pokemon.height / 10).toFixed(
    1
...
async function displayPokemons(searchQuery = '') {
  ...
  if (searchQuery) {
    ...
    const pokemon = isId
      ? await fetchPokemonById(searchQuery) // Fetch by ID
